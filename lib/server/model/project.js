...
import ThirdPartDB from './entities/third-part-db.js'
...
export default {
            ...
            if (projectData.copyFrom) {
                ...
                const [
                    ...
                    getRepository(Form)
                        ...
                        .getMany(),
                    getRepository(ThirdPartDB)
                        .createQueryBuilder('thirdPartDB')
                        .where('thirdPartDB.projectId = :projectId', whereCopyForm)
                        .getMany()
                ])
                ...
                if (projectDataTables.length) {
                    const mysqlTables = projectDataTables.filter(projectDataTable => projectDataTable.thirdPartDBId <= 0)
                    const thirdPartDBTables = projectDataTables.filter(projectDataTable => projectDataTable.thirdPartDBId > 0)
                    ...
                    // 复制第三方数据源
                    const copyThirdPartDBs = getRepository(ThirdPartDB).create(projectThirdPartValues.map(item => {
                        const { id, createTime, updateTime, ...others } = item
                        others.projectId = projectId
                        return others
                    }))
                    const newThirdPartDBs = await transactionalEntityManager.save(copyThirdPartDBs)
                    const thirdPartDBIdMap = {}
                    newThirdPartDBs.forEach((newThirdPartDB, index) => {
                        thirdPartDBIdMap[projectThirdPartValues[index]?.id] = newThirdPartDB.id
                    })
                    ...
                    newDataTableModifyRecords.forEach((newDataTableModifyRecord) => {
                        if (dataTableIdThirdDBIdMap[newDataTableModifyRecord.tableId] <= 0) {
    ...
    updateProject (id, fields = {}) {
        const data = Object.assign({}, fields, { id })
        const saveData = getRepository(Project).create(data)
        return getRepository(Project).save(saveData)
    },
