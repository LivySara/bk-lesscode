...
import { walkGrid, uuid, sm4Encrypt } from '../util'
import { METHODS_WITHOUT_DATA, parseValue2UseScheme } from '../../shared/api'
import { transformNocodeFields } from '../../shared/form-field-protocol-transform'
...
import PreviewDb from '../model/entities/preview-db'
import Flow from '../model/entities/flow'
...
const apiArr = [
    ...
    'updateFunctionUrl',
    'modifyMobileNavLayout',
    'updateLayoutInstRoutePath',
    'updateApiHeader',
    'updateCrypto',
    'transformVantDatePicker',
    'addMobileSideLayout',
    'updateMarkdownPage',
    'updateNoCodeFormPage',
    'updateNoCodeDataManagePage',
    'updateH5ContainerEffect',
    'updateVue3InteractiveCompsTransferProperty'
]
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const layoutInstList = projectList.map(project => {
                ...
                return {
                    ...
                    routePath: '/mobile/',
                    isDefault: 1,
                    showName: global.i18n.t('移动端空白布局'),
...
// 内置官方自定义组件
async function initCustomComps () {
    try {
        // 这里importList里每一项设计成对象， 方便后续支持更多属性，比如framework、compType, version， 新增/update等
        const sucList = await initResources({
            comp: {
                importList: [
        ...
        return {
            code: 0,
            message: global.i18n.t('初始化自定义组件成功')
        }
    } catch (error) {
        console.log(error, 'err')
        return {
            code: -1,
            message: global.i18n.t('初始化自定义组件失败: {{n}}', { n: error }),
            data: null
        }
    }
}
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const iamAppPermList = projectList.map(project => {
                ...
                return {
                    projectId: id,
                    systemId: systemId,
                    systemName: global.i18n.t('蓝鲸运维开发平台-{{n}}', { n: projectName }),
                    systemNameEn: `lesscode-${projectCode}`,
                    systemDesc: global.i18n.t('蓝鲸运维开发平台-{{n}}', { n: projectName }),
...
// Updates a layout in a layout repository.
async function modifyMobileNavLayout () {
    try {
        const layoutRepository = getRepository(Layout)
        const layoutTemplates = await layoutRepository.find({ layoutType: 'MOBILE' })

        layoutTemplates.forEach(layout => {
            if (layout.defaultPath === '/mobile-bottom-nav') {
                layout.defaultPath = '/mobile/bottom-nav'
            }

            if (layout.defaultPath === '/mobile/') {
                layout.defaultPath = '/mobile/empty-layout'
            }
        })

        await layoutRepository.save(layoutTemplates)
        return {
            code: 0,
            message: 'layoutRepo 更新成功，mobile-bottom-nav已更新为/mobile/bottom-nav',
            data: null
        }
    } catch (error) {
        return {
...
async function updateLayoutInstRoutePath () {
    try {
        const layoutInstRepo = getRepository(LayoutInst)
        const layoutInstList = await layoutInstRepo.find({ layoutType: 'MOBILE' })
        layoutInstList.forEach(layout => {
            if (!layout.routePath.includes('/mobile/')) {
                layout.routePath = `/mobile${layout.routePath}`
            } else if (layout.routePath === '/mobile/') {
                layout.routePath = '/mobile/empty-layout'
            }
        })
        await layoutInstRepo.save(layoutInstList)
        return {
            code: 0,
            message: 'layoutInst更新成功,更新了routePath'
        }
    } catch (error) {
        return {
            code: -1,
...
async function updateApiHeader () {
    try {
        const funcRepository = getRepository(Func)
        const funcList = await funcRepository.find()
        ...
        const modifyData = (data) => {
            return Object.prototype.toString.apply(data) === '[object Object]' ? [] : data
        }
        for (const key in funcList) {
            const funcData = funcList[key]
            const apiHeader = modifyData(funcData.apiHeader)
            await funcRepository.update(funcData.id, {
                apiHeader,
                updateTime: funcData.updateTime,
                updateUser: funcData.updateUser
        ...
        for (const key in apiList) {
            ...
            const header = modifyData(api.header)
            await apiRepository.update(api.id, {
                header,
        ...
        return {
            code: 0,
            message: `更新header成功：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
async function updateCrypto () {
    try {
        const previewDbRepository = getRepository(PreviewDb)
        const previewDbs = await previewDbRepository.find()

        const modifyData = (text) => {
            if (text.includes('::lesscode::')) {
                const [iv, hashText] = text.split('::lesscode::')
                ...
                return sm4Encrypt(decrpyted.toString())
            }
            return text
        }
        for (const key in previewDbs) {
            const previewDb = previewDbs[key]
            const userName = modifyData(previewDb.userName)
            const passWord = modifyData(previewDb.passWord)
            await previewDbRepository.update(previewDb.id, {
                userName,
                passWord,
                updateTime: previewDb.updateTime,
                updateUser: previewDb.updateUser
            })
        }
        return {
            code: 0,
            message: `更新国密成功：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
/** 更新widgeht-vant-date-time-picker => widget-van-date-picker */

async function transformVantDatePicker () {
    try {
        // 获取Page仓库实例
        const pageRepository = getRepository(Page)
        // 获取所有Page数据
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const taskList = pageList.map(pageData => {
                ...
                // 修改特定节点类型为'widget-van-picker'
                const modifyVanPickerType = (node) => {
                    if (node?.type === 'widget-van-date-time-picker') {
                        node.type = 'widget-van-picker'
                        node.name = 'widget-van-datetime-picker'
                        node.renderProps.componentType = {
                            'format': 'value',
                            'code': 'van-datetime-picker',
                            'renderValue': 'van-datetime-picker',
                            'valueType': 'hidden',
                            'payload': {}
                        }
                        if (Object.hasOwnProperty.call(node.renderProps.value, 'modifiers')) {
                            delete node.renderProps.value.modifiers
                        }
                        node.renderProps.value.directive = 'v-model'
                        node.renderProps.format = {
                            'format': 'value',
                            'code': 'yyyy-MM-DD hh:mm:ss',
                            'renderValue': 'yyyy-MM-DD hh:mm:ss',
                            'valueType': 'string',
                            'payload': {}
                        }
                    }
                    ...
                    Object.keys(renderSlots).forEach((key) => {
                        const renderSlot = Array.isArray(renderSlots[key]) ? renderSlots[key] : [renderSlots[key]]
                        renderSlot.forEach(modifyVanPickerType)
                    })
                }
                // 遍历目标数据并修改节点类型
                targetData.forEach(modifyVanPickerType)
                // 更新Page数据
                return transactionalEntityManager.update(Page, {
                    id: pageData.id
                }, {
        ...
        return {
            code: 0,
            message: `transformVanPicker success：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
// add a mobile-side layout in layout repository
async function addMobileSideLayout () {
    try {
        const layoutRepository = getRepository(Layout)
        
        // create a mobile-side layout
        const mobileSideLayout = new Layout()
        mobileSideLayout.layoutType = 'MOBILE'
        mobileSideLayout.defaultPath = '/mobile/side-nav'
        mobileSideLayout.defaultName = '移动端侧边导航'
        mobileSideLayout.defaultCode = 'mobileSideView'
        mobileSideLayout.defaultContent = JSON.stringify({
            'logo': '',
            'menuList': [
                {
                    'name': '首页',
        ...
        mobileSideLayout.type = 'mobile-side'
        await layoutRepository.save(mobileSideLayout)
...
async function updateMarkdownPage (name) {
    try {
        const pageRepository = getRepository(Page)
        const allPageData = await pageRepository.find({ nocodeType: 'MARKDOWN' })
        allPageData.forEach(page => {
            let targetData = {}
            try {
                targetData = (typeof page.content) === 'string' ? JSON.parse(page.content) : page.content
            ...
            const mdCon = targetData.content || ''

            const newTargetData = []
            const uid = uuid(5)
            newTargetData.push({
                type: 'widget-md-editor',
                complex: false,
                componentId: `md-editor-${uid}`,
                custom: false,
                interactive: false,
                name: 'md-editor',
                renderAlign: {},
                ...
                renderSlots: {},
                renderStyles: { height: '100%', minHeight: '500px' }
            })
            
            page.content = JSON.stringify(newTargetData)
            page.nocodeType = ''
            page.updateBySystem = true
        })
        await pageRepository.save(allPageData)
        return {
            code: 0,
            message: global.i18n.t('markdown页面数据更新成功')
        }
    } catch (error) {
        return {
            code: -1,
...
// 更新form表中字段协议
// 1. 只转换nocode表单页绑定且未被流程复用的表单
// 2. lesscode表单容器不能引用或复用流程表单，数据管理容器不能绑定流程表单
// 3. 流程提单页表单或人工节点表单不能引用、复用lesscode表单
async function updateNoCodeFormPage () {
    try {
        const pageRepository = getRepository(Page)
        const formPages = await pageRepository.find({ nocodeType: 'FORM' })
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            // 找到当前被流程节点中绑定的表单id，将它从待更新列表中删除
            flows.forEach(flow => {
                if (flow.formIds) {
                    let nodeFormMap
                    try {
                        nodeFormMap = JSON.parse(flow.formIds)
                    } catch (e) {
                        nodeFormMap = {}
                    }
                    const formIdsInFlowNode = new Set(Object.values(nodeFormMap))
                    formIdsInFlowNode.forEach(id => {
                        if (typeof id === 'number') {
                            const index = formPages.findIndex(page => page.formId === id)
                            if (index > -1) {
            ...
            for (let i = 0; i < formPages.length; i++) {
                ...
                const pageContent = {
                    componentId: `form-container-${uuid(5)}`,
                    ...
                    renderProps: {
                        ...
                        actions: {
                            format: 'value',
                            code: { submit: true, reset: true },
                            renderValue: { submit: true, reset: true },
                            valueType: 'hidden',
                            payload: {}
                        }
                    },
                ...
                if (page.formId) {
                    const forms = await formRepository.find({ id: page.formId })
                    if (forms.length === 1) {
                        const form = forms[0]
                        const fields = JSON.parse(form.content)
                        const newProtocolFields = transformNocodeFields(fields)
                        pageContent.renderProps.dataSource.code.id = form.id
                        pageContent.renderProps.dataSource.code.tableName = form.tableName
                        pageContent.renderProps.dataSource.renderValue.id = form.id
                        pageContent.renderProps.dataSource.renderValue.tableName = form.tableName
                        pageContent.renderProps.fields.code = newProtocolFields
                        pageContent.renderProps.fields.renderValue = newProtocolFields
                        taskList.push(transactionalEntityManager.update(Form, {
                            id: form.id
                        }, {
                            pageId: page.id,
                            componentId: pageContent.componentId,
                            content: JSON.stringify(newProtocolFields)
                        }))
                    }
                }
                taskList.push(transactionalEntityManager.update(Page, {
                    id: page.id
                }, {
                    nocodeType: null,
        ...
        return {
            code: 0,
            message: `更新表单页以及对应的form表成功 ${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
// 将nocode表单数据管理页转化为自定义页面中的数据管理容器
async function updateNoCodeDataManagePage () {
    try {
        const pageRepository = getRepository(Page)
        const dataManagePages = await pageRepository.find({ nocodeType: 'FORM_MANAGE' })
        dataManagePages.forEach(page => {
            let content
            try {
            ...
            const { filters = [], tableColsExclude = [], buttons = [], tableActions = [] } = content

            const newContent = [{
                componentId: `data-manage-container-${uuid(5)}`,
                ...
                renderProps: {
                    ...
                    tableRowActions: {
                        ...
                        format: 'value',
                        payload: {},
                        renderValue: tableActions,
                        valueType: 'color'
                    },
                    tableColsExclude: {
                        code: tableColsExclude,
                        format: 'value',
                        payload: {},
                        renderValue: tableColsExclude,
                        valueType: 'color'
                    }
                },
                renderSlots: {},
            ...
            page.nocodeType = null
            page.formId = null
            page.content = JSON.stringify(newContent)
            page.updateBySystem = true
        })
        await pageRepository.save(dataManagePages)
        return {
            code: 0,
            message: global.i18n.t('数据管理页面数据更新成功')
        }
    } catch (error) {
        return {
            code: -1,
...
async function updateH5ContainerEffect () {
    try {
        const pageRepository = getRepository(Page)
        const allPageData = await pageRepository.find()
        allPageData.forEach(page => {
            ...
            targetData.forEach((grid, index) => {
                if (grid.type === 'h5-container' && grid.renderProps.effect.code === 'creative') {
                    grid.renderProps.effect = {
                        'format': 'value',
                        'code': 'slide',
                        'renderValue': 'slide',
                        'valueType': 'string',
        ...
        return {
            code: 0,
            message: global.i18n.t('H5容器effect属性更新成功')
        }
    } catch (error) {
        return {
            code: -1,
...
async function updateVue3InteractiveCompsTransferProperty () {
    try {
        const pageRepository = getRepository(Page)
        const allPageData = await pageRepository.find()
        allPageData.forEach(page => {
            ...
            targetData.forEach((grid, index) => {
                if (['bk-dialog', 'bk-sideslider'].includes(grid.type) && grid.renderProps.transfer.valueType === 'string') {
                    grid.renderProps.transfer = {
                        'format': 'value',
                        'code': grid.renderProps.transfer.code === 'body',
                        'renderValue': grid.renderProps.transfer.code === 'body',
                        'valueType': 'boolean',
                        'payload': {}
                    }
                }
        ...
        return {
            code: 0,
            message: global.i18n.t('Vue3交互组件transfer属性更新成功')
        }
    } catch (error) {
        return {
            code: -1,
